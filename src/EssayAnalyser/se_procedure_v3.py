...
# Function: pre_process_ass_q(ass_q,dev)
...
def pre_process_text_se(text0,nf,nf2,dev):
#def pre_process_essay(text0,struc_feedback,nf,nf2,dev,model):    
    ...
    parasenttok = [PunktSentenceTokenizer().tokenize(item) for item in paratok]     # xxxx normal NLTK sentence splitter   
...
def pre_process_struc(text,nf,nf2,dev):
    ...
    text50 = find_and_label_headings(text)
    ...
    section_names = ['Headings', 'Title', 'Introduction', 'Conclusion', 'Summary', 'Preface', 'Numerics', 'Punctuation']
    section_labels = ['#-s:h#', '#-s:t#', '#+s:i#', '#+s:c#', '#+s:s#', '#+s:p#', '#-s:n#', '#-s:p#', ]
    ...
    return text,section_names,section_labels,headings,conclheaded,c_first,c_last,introheaded,i_first,i_last
...
def pre_process_ass_q(ass_q,dev):
    ass_q,a01,a02,a03,a04 = pre_process_text_se(ass_q,[],[],[]) 
    ...
    ass_q = process_sents(get_lemmas, temp)
