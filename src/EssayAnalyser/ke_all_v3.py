...
# Function: add_item_to_array_ke(myarray, num, item)
# Function: add_all_node_edges_ke(gr, text)
...
# Called by 'process_essay_ke' (in this file).
def add_item_to_array_ke(myarray, num, item):
    ...
    while 1:
        ...
        if len(window_words) == 2:
            try: # 'add_edge' is defined in 'pygraph\classes\digraph'
                gr.add_edges_from([(window_words[0][1], window_words[1][1])]) # add the edges between the key lemmas not the inflected real words
...
def keywords2ngrams(keywords,text,n):
    ...
    mylist3 = []
    for item in tuples:
        if item not in mylist3: # Get rid of duplicate ngrams 
            mylist3.append(item) 
        else:
            mylist3 = mylist3
...
# Function: cf_ass_q_keylemmas(ass_q,keylemmas,scoresNfreqs)
# Compares the assignment question with the found key lemmas.
# Returns the number of key lemmas that occur in the assignment question (sum_kls_in_ass_q)
...
def process_essay_ke(text,wordtok_text,nf,nf2,dev):
    ...
    for item in unique_lemma_set:  # item: fox 
        ...
        add_item_to_array_ke(myarray, item, temp)

    # Initiate an empty directed graph 'gr' of 'digraph' class  
    gr=nx.DiGraph()
    #gr=nx.Graph() # xxxx for non-directed graph testing
    
    # Add nodes to the directed graph 'gr', one node for each unique lemma.
    ...
    gr.add_nodes_from(unique_lemma_set)
    ...
    for item in keylemmas:
        wordlist = myarray[item] # by looking up in myarray the values of the key lemmas
        monitorlist = []
        for w in wordlist:
...
def debora_write_results_ke(text_ke,text_se,gr,di,myarray,\
    ...
    nf.write(str(keylemmas))
    nf.write('\n')    
    nf.write('\nKey words (the inflected forms of the key lemmas):\n')
    nf.write(str(keywords))
    nf.write('\n')    
...
##    nf.write('\n')    
    ...
    nf.write(str(quadgram_keyphrases))
    nf.write('\n')             
    nf.write('\nTrigrams:\n')
    ...
    nf.write('\n')
    nf.write('\nKey lemmas (+ freq) in ass_q_long:\n')
    nf.write(str(kls_in_ass_q_long))
    nf.write('\n')
    ...
    nf.write('\n')
    nf.write('\nKey lemmas (+ freq) in ass_q_short:\n')
    nf.write(str(kls_in_ass_q_short))
    nf.write('\n')
    ...
    # nf2.write('\n')
    # nf2.write('Quadgrams:\n') #  (seqs of within-sentence key words)
    # nf2.write(str(quadgram_keyphrases))
    # nf2.write('\n')    
    # nf2.write('Trigrams:\n')
